...
use trackmaker_rs::audio::psk::{
    PskDemodulator, PskModulator, utils as psk_utils,
};
...
impl PerformanceMetrics {
    ...
    fn print_summary(&self) {
        ...
        println!(
            "   Frame Encoding:      {:>8.2} ms",
            self.encoding_time
                .as_secs_f64()
                * 1000.0
        );
        println!(
            "   PSK Modulation:      {:>8.2} ms",
            self.modulation_time
                .as_secs_f64()
                * 1000.0
        );
        println!(
            "   Signal Generation:   {:>8.2} ms",
            self.signal_generation_time
                .as_secs_f64()
                * 1000.0
        );
        println!(
        ...
        println!(
            "   PSK Demodulation:    {:>8.2} ms",
            self.demodulation_time
                .as_secs_f64()
                * 1000.0
        );
        println!(
            "   Text Reconstruction: {:>8.2} ms",
            self.text_reconstruction_time
                .as_secs_f64()
                * 1000.0
        );
        println!(
            "   Total Processing:    {:>8.2} ms",
        ...
        println!(
            "   Sample Rate:         {:>8.2} MSamples/s",
            (self.signal_samples as f64 / 1_000_000.0)
                / self.total_time.as_secs_f64()
        );
        ...
        println!(
            "   Frame Loss Rate:     {:>8.2}%",
            self.frame_loss_rate * 100.0
        );
        println!(
            "   Byte Error Rate:     {:>8.2}%",
            self.byte_error_rate * 100.0
        );
        println!(
            "   Character Accuracy:  {:>8.2}%",
            self.character_accuracy * 100.0
        );
...
fn main() {
    ...
    let demodulator =
        PskDemodulator::new(sample_rate, carrier_freq, symbol_rate);
    ...
    let text_message = match fs::read_to_string(text_file_path) {
        ...
        Err(e) => {
            println!(
                "Failed to read file {}: {}, using fallback text",
                text_file_path, e
            );
    ...
    println!(
        "Original text: {} chars, {} bytes",
        text_message.chars().count(),
        text_bytes.len()
    );
    ...
    let total_frames =
        (text_bytes.len() + bytes_per_frame - 1) / bytes_per_frame;
    ...
    // Generate preamble
    let preamble =
        psk_utils::generate_chirp_preamble(sample_rate, 2000.0, 10000.0, 440);
    ...
    for frame_idx in 0..total_frames {
        ...
        let end_byte =
            std::cmp::min(start_byte + bytes_per_frame, text_bytes.len());
        ...
        for byte_idx in start_byte..end_byte {
            ...
            for bit_idx in 0..8 {
                if bit_start + bit_idx < 96 {
                    frame_bits[bit_start + bit_idx] =
                        ((byte_value >> (7 - bit_idx)) & 1) as u8;
                }
            }
        }
    ...
    for (frame_idx, frame_wave) in modulated_frames
        .iter()
        .enumerate()
    {
    ...
    let correlation_threshold = correlation
        .iter()
    ...
    for (frame_idx, &frame_start) in frame_starts
        .iter()
        .enumerate()
    {
                ...
                if frame_id > 0 {
                    ...
                    for byte_idx in 0..11 {
                        ...
                        for bit_idx in 0..8 {
                            ...
                            if bit_pos < demodulated_bits.len()
                                && demodulated_bits[bit_pos] == 1
                            {
                    ...
                    if frame_idx < 5 || frame_idx >= frame_starts.len() - 5 {
                        println!(
                            "Frame {}: ID={}, demodulated successfully",
                            frame_idx + 1,
                            frame_id
                        );
                    } else if frame_idx == 5 {
    ...
    metrics.frame_loss_rate = if metrics.transmitted_frames > 0 {
        (metrics.transmitted_frames - metrics.received_frames) as f32
            / metrics.transmitted_frames as f32
    } else {
        0.0
    };
    ...
    if !received_frames.is_empty() {
        ...
        for (frame_id, data_bytes) in received_frames {
            if frame_id <= 5 || frame_id > metrics.transmitted_frames as u8 - 5 {
                println!(
                    "Processing frame ID={}, data={:?}",
                    frame_id, data_bytes
                );
            } else if frame_id == 6 {
                println!(
                    "... (processing frames 6-{}) ...",
                    metrics.transmitted_frames - 5
                );
            }
        ...
        match String::from_utf8(reconstructed_text.clone()) {
            Ok(text) => {
                ...
                let min_len =
                    std::cmp::min(original_chars.len(), received_chars.len());
                for i in 0..min_len {
                ...
                let min_byte_len =
                    std::cmp::min(original_bytes.len(), received_bytes.len());
                for i in 0..min_byte_len {
                    if original_bytes[i] != received_bytes[i] {
                        byte_errors += 1;
                    }
                }
                byte_errors += (original_bytes.len() as i32
                    - received_bytes.len() as i32)
                    .abs() as usize;
                ...
                if text.len() > 200 {
                    println!(
                        "{}...\n[Text truncated, showing first 200 chars]",
                        &text[..200]
                    );
                } else {
                    println!("{}", text);
                ...
                if text == text_message {
                    ...
                    for (i, (orig_char, recv_char)) in original_chars
                        .iter()
            ...
            Err(e) => {
                ...
                println!(
                    "Raw bytes: {:?}",
                    &reconstructed_text
                        [..std::cmp::min(50, reconstructed_text.len())]
                );
