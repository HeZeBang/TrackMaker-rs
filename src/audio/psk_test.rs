use crate::audio::psk::{PskDemodulator, PskModulator, utils as psk_utils};
...
fn test_direct_vector_transmission() {
    ...
    for (_frame_idx, &frame_start) in frame_starts
        .iter()
        .enumerate()
    {
    ...
    if !frame_starts.is_empty() {
        ...
        let success_rate =
            (correct_frame_num as f32 / frame_starts.len() as f32) * 100.0;
        ...
        assert_eq!(
            correct_frame_num, 100,
            "Should correctly demodulate all frames"
        );
        assert!(
...
fn test_text_file_transmission() {
    ...
    let total_frames =
        (text_bits.len() + data_bits_per_frame - 1) / data_bits_per_frame;
    ...
    for frame_idx in 0..total_frames {
        ...
        let end_bit =
            std::cmp::min(start_bit + data_bits_per_frame, text_bits.len());

        for (i, &bit) in text_bits[start_bit..end_bit]
            .iter()
            .enumerate()
        {
    ...
    for &frame_start in &frame_starts {
        ...
        if frame_end <= output_track.len() {
            ...
            if demodulated_bits.len() >= header_bits {
                ...
                if frame_num > 0
                    && frame_num <= total_frames as u16
                    && total_frames_decoded == total_frames as u16
                {
                    let data_bits: Vec<u8> = demodulated_bits[header_bits
                        ..std::cmp::min(bits_per_frame, demodulated_bits.len())]
                        .to_vec();
    ...
    let reconstructed_text =
        String::from_utf8_lossy(&reconstructed_bytes).to_string();

    // Compare results
    let mut byte_errors = 0;
    for (&orig, &recon) in text_bytes
        .iter()
    ...
    println!(
        "Reconstructed text length: {} bytes",
    ...
    println!(
        ...
        byte_errors,
        text_bytes.len(),
        (byte_errors as f32 / text_bytes.len() as f32) * 100.0
    );
    ...
    assert_eq!(
        received_frames.len(),
        total_frames,
        "Should decode all frames correctly"
    );
    assert_eq!(byte_errors, 0, "Should have no byte errors");
    assert_eq!(
        test_text, reconstructed_text,
        "Reconstructed text should match original exactly"
    );
}
...
fn test_think_different_file_transmission() {
    ...
    if byte_errors == 0 {
        println!(
            "✅ Perfect reconstruction! Think Different text matches exactly."
        );
    } else {
        println!("❌ Text reconstruction has errors.");
    }
