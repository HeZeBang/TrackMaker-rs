...
impl PskModulator {
    ...
    pub fn modulate_bpsk(&self, bits: &[u8]) -> Vec<f32> {
        ...
        for (symbol_idx, &bit) in bits.iter().enumerate() {
            ...
            for sample_idx in 0..self.samples_per_symbol {
                let t = (symbol_idx * self.samples_per_symbol + sample_idx)
                    as f32
                    / self.sample_rate;
    ...
    pub fn modulate_qpsk(&self, bits: &[u8]) -> Vec<f32> {
        let mut signal =
            Vec::with_capacity((bits.len() / 2) * self.samples_per_symbol);
        ...
        for chunk in bits.chunks(2) {
            ...
            for sample_idx in 0..self.samples_per_symbol {
                ...
                let t = (symbol_start * self.samples_per_symbol + sample_idx)
                    as f32
                    / self.sample_rate;
                let phase = 2.0 * PI * self.carrier_freq * t + phase_offset;
    ...
    pub fn demodulate_bpsk(&self, signal: &[f32]) -> Vec<u8> {
        ...
        for symbol_idx in 0..num_symbols {
            ...
            if end <= signal.len() {
                ...
                let corr_0: f32 = symbol_samples
                    .iter()
                    .zip(self.ref_signal_0.iter())
                    .map(|(s, r)| s * r)
                    .sum();

                let corr_1: f32 = symbol_samples
                    .iter()
    ...
    pub fn demodulate_bpsk_phase(&self, signal: &[f32]) -> Vec<u8> {
        ...
        for symbol_idx in 0..num_symbols {
            ...
            if end <= signal.len() {
                ...
                for (i, &sample) in symbol_samples
                    .iter()
                    .enumerate()
                {
...
mod tests {
    ...
    fn test_bpsk_modulation_demodulation() {
        ...
        let modulator =
            PskModulator::new(sample_rate, carrier_freq, symbol_rate);
        let demodulator =
