...
impl ReedSolomonEncoder {
    ...
    pub fn encode(&self, data: &[u8]) -> Vec<u8> {
        debug!(
            "Encoding {} bytes with RS({}, {})",
            data.len(),
            data.len() + self.ecc_len,
            data.len()
        );
...
impl ReedSolomonDecoder {
    ...
    pub fn decode(
        ...
        if encoded_data.len() < self.ecc_len {
            error!(
                "Encoded data too short: {} bytes, expected at least {}",
                encoded_data.len(),
                self.ecc_len
            );
        ...
        match self
            .decoder
            .correct(&mut corrupted, known_erasures)
        {
            Ok(corrected) => {
                ...
                debug!(
                    "Successfully corrected {} errors, recovered {} bytes",
                    errors_corrected,
                    data.len()
                );
