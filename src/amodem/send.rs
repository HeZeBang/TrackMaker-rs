...
use crate::amodem::{
    ...
    dsp::Modem,
    equalizer::{Equalizer, get_prefix, SILENCE_LENGTH},
...
impl<W: Write> Sender<W> {
    ...
    pub fn write_samples(&mut self, samples: &[f64]) -> std::io::Result<()> {
        let scaled_samples: Vec<f64> = samples.iter().map(|&s| s * self.gain).collect();
    ...
    pub fn write_complex(&mut self, samples: &[Complex64]) -> std::io::Result<()> {
        // Python version: sym.real * scaling, matching common.dumps()
        let real_samples: Vec<f64> = samples.iter().map(|c| c.re * self.gain).collect();
    ...
    pub fn start(&mut self) -> std::io::Result<()> {
        ...
        for &value in &prefix {
            let pilot_signal: Vec<Complex64> = self.pilot.iter()
                .map(|&c| c * value)
                .collect();
        ...
        let symbols = self.equalizer.train_symbols(crate::amodem::equalizer::EQUALIZER_LENGTH, &self.config);
        let signal = self.equalizer.modulator(&symbols);
    ...
    pub fn modulate(&mut self, bits: impl Iterator<Item = bool>) -> std::io::Result<()> {
        let all_bits: Vec<bool> = bits.chain(self.padding.iter().copied()).collect();
        let symbols = self.modem.encode(all_bits.into_iter());
        ...
        for (i, symbol_chunk) in iterate(symbols.into_iter(), nfreq).enumerate() {
            ...
            for (j, &symbol) in symbols.iter().enumerate() {
                if j < nfreq {
                    for (k, &carrier) in self.carriers[j].iter().enumerate() {
...
pub fn send<R: std::io::Read, W: Write>(
    ...
    // Pre-padding with silence
        let silence_samples = (config.fs * (config.silence_start + extra_silence)) as usize;
    ...
    eprintln!("Sending {:.3} seconds of training audio", training_duration as f64 / config.fs);
