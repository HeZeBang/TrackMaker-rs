...
pub fn dumps_with_metadata(samples: &[f64], sample_rate: u32, channels: u16, bit_depth: u16) -> Vec<u8> {
    let mut result = Vec::new();
...
pub fn loads_with_metadata(data: &[u8]) -> Result<(PcmMetadata, Vec<f64>), String> {
    if data.len() < 16 {
