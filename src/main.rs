...
fn main() {
    ...
    let selections = &["Sender", "Receiver", "Test (Direct Vec)"];
    ...
    if selection == 0 {
        ...
        // Test (Direct Vec)
        run_test_direct_vec(sample_rate as u32);
    }
...
fn run_test_direct_vec(sample_rate: u32) {
    info!("Running direct vector test...");
    ...
    info!("Generated signal length: {} samples", output_track.len());

    // Now demodulate the generated signal (same as receiver logic)
    let mut correct_frame_num = 0;
    let samples_per_symbol = (sample_rate_f32 / symbol_rate) as usize;
    let frame_length_samples = 108 * samples_per_symbol; // 108 bits per frame (100 + 8 CRC)
    let preamble_length = preamble.len();
        ...
        if frame_end <= output_track.len() {
            ...
            if demodulated_bits.len() >= 8 {
                ...
                if frame_id > 0 && frame_id <= 100 {
                    ...
                    if bit_errors > 0 {
                        info!("  Frame {}: {} bit errors in data", frame_id, bit_errors);
                    }
                } else {
                    info!("Frame {}: Error in frame, decoded ID: {}", frame_idx + 1, frame_id);
                }
    ...
    info!("=== Test Results ===");
    info!("Total Generated Frames: 100");
    info!("Total Detected Frames: {}", frame_starts.len());
    info!("Total Correct Frames: {}", correct_frame_num);
    ...
    if !frame_starts.is_empty() {
        ...
        info!("Frame Detection Rate: {:.1}%", detection_rate);
        info!("Demodulation Success Rate: {:.1}%", success_rate);
